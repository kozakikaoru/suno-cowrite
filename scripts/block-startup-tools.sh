#!/bin/bash
# PreToolUse hook for the "suno-artist-production" plugin.
# 2 種類の物理ブロックを担当する:
#   1. トリガーターン (=/suno-artist-production:studio の起動直後ターン) は
...
if [ -f "$TRIGGER_MARKER" ]; then
  cat >&2 <<EOF
🚨 [suno-artist-production] studio 起動直後のターンは '${TOOL_NAME}' ツールは使えません。
...
if [ "$TOOL_NAME" = "AskUserQuestion" ] && [ -f "$ARTIST_ROOT/.production/ACTIVE" ]; then
  cat >&2 <<EOF
🚨 [suno-artist-production] マネージャーモード起動中は AskUserQuestion (クリック式選択肢UI) は使えません。
