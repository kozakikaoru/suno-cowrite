...
Suno (suno.com) 用の **単曲生成** プラグイン。マネージャー (受付ペルソナ) が窓口になり、**2 職種のサブエージェント** にタスクを振り分けて、Suno 楽曲を 1 曲ずつ企画・制作します。

**設計思想**: 制作は 1 起動で完結し、`artist.yaml` などの事前準備は要らない。**オリジナリティ厳守** — 実在曲の丸パクリや歌詞の引用はしない (参照資料は骨格だけを借りる 2 速度モデル)。出力は Suno Custom Mode の全設定値 — **Style / Lyrics プロンプト + 各種パラメータ** を「欄ごとに貼れる形式 + 機械可読 JSON」で渡す。
...
> 夏っぽい切ない曲を 1 曲つくって          # ターン 2: ここから制作 (oneshot フローへ)
```

以後は「1 曲作って」「サビ直して」「仕様変わった?」のように自然言語で話しかければ、マネージャーが適切なサブエージェント / スキルへ振り分ける。呼び名 (「〇〇P」) は質問せず自動判定する (`artist.yaml` の `producer_name` → アカウント表示名 → OS のフルネーム → git ユーザー名 → `$USER` の優先順)。
...
| `:oneshot` | 単発曲制作 (簡易ブリーフ → 一括制作 → 機械検証 → 入稿セット。既定では保存しない) |
...
**セッション状態 = `.production/` に隠す / 成果物 (入稿セット) = チャット提示、希望時のみ保存**。
