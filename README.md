...
/plugin install suno-cowrite@suno-artist-marketplace
/reload-plugins
```

`/plugin list` で `suno-cowrite` が表示されればインストール完了。以後 `/suno-cowrite:studio` で起動できる。
...
**`/suno-cowrite:studio`** で起動 (起動方法はこれのみ)。単曲制作用の作業ディレクトリで始めるのがおすすめ:

```
mkdir -p ~/Music/suno-songs && cd ~/Music/suno-songs
claude
> /suno-cowrite:studio        # ターン 1: マネージャーが状況付きで挨拶だけ返す
...
| コマンド | 用途 |
|---|---|
| `/suno-cowrite:studio` | 起動 (メイン入口)。2 段階起動、以後は自然言語で全ユースケース |
...
**ターン 1** (`/suno-cowrite:studio` コマンドだけ): 挨拶テキストのみ、ツール使用ゼロ。hook が注入した状況 (`.production/state.md`) を使った「状況付き挨拶」を返す (`PreToolUse` フックが Bash/Read/Grep/Glob/AskUserQuestion を物理拒否)。
...
```
無ければ未起動 → `/suno-cowrite:studio` で起動。
