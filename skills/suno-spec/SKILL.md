...
| 層 | パス | 性質 |
|---|---|---|
| 上書き版 | `${XDG_CONFIG_HOME:-~/.config}/suno-artist-production/suno-spec.md` | update-spec フローが保存する更新版。プラグイン更新で消えず、全アーティストで共有 |
...
- **style-vocab.md も同じ 2 層構造・同じ判定規則・同じ調査日契約**で実効版を決める (上書き版: `${XDG_CONFIG_HOME:-~/.config}/suno-artist-production/style-vocab.md`。実効パスと鮮度は hook が spec と並べて毎ターン注入する)。ただし鮮度の声かけ閾値は spec と別で、調査日から **90 日超過**のときに再調査 (update-spec、対象: style-vocab) を提案する。

## 誰がいつ読むか

| 読む人 | タイミング | 読む範囲 |
|---|---|---|
| composer | **曲作りごとに毎回** (モデル・Style・Exclude・スライダー・Persona 適用を決める前) | 全体。特に §1〜§3 (モデル/上限/設定値)・§5 (Style 記法)・§6 (Persona 運用) |
| composer (style-vocab) | Style のムード・情景・質感の枠を決めるとき | 通読しない。該当カテゴリ (§2〜§6) だけ拾い読み |
| lyricist | 歌詞執筆と入稿版変換の前 | §4 (メタタグ語彙)・§7 (日本語入稿対策。日本語アーティストのみ) |
...
- サブエージェントはプラグインのファイルパスを知らない。マネージャーは composer / lyricist の起動プロンプトに**実効 spec の絶対パス**を必ず含める。

## 更新フロー (概要)

詳細手順は update-spec スキル (`/suno-artist-production:update-spec`) を参照。style-vocab の更新も同スキルの「対象の決定」に従う (researcher は対象別に実行)。以下は spec の例。
