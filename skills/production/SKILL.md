---
description: 制作ノウハウ参照資料集 (スラッシュ起動用ではなく参照コンテナ)。制作エージェント (songsmith) が 1 曲を一括制作 (Style + 歌詞 + 韻 + タイトル) する前に読む「制作コア」束ねと、その読み方ガイド。作曲必読 (composing/02) + 作詞必読 (songwriting 01/05/06/07/08/11) を 1 ファイルに連結した write_core.md、韻必読 (songwriting 03/09) を連結した rhyme_core.md の引き方を定義する。「制作の必読はどこ?」「1 曲まとめて作るとき何を読む?」のような参照で読まれる。
---
