...
- **composing 資料の絶対パス**: `<プラグインルート>/skills/composing/SKILL.md` と `<プラグインルート>/skills/composing/references/` (読み方ガイドは SKILL.md 側。「無ければ spec のみで進めてよい」と添える)
...
- **語り主導 (ポエトリー) の曲のみ** `<プラグインルート>/skills/composing/references/06_poetry-spoken.md`、**K-POP 系の曲のみ** `<プラグインルート>/skills/composing/references/04_kpop.md` の絶対パスも渡す (「『ことば側への申し送り』節を必読。ファイルが無ければ従来資料のみで進めてよい」と添える)
- 返してもらうもの: 作品版歌詞 / 入稿版歌詞 (構成タグ英語 + 変換済み本文) / タイトル案 / オリジナリティ自己申告 (中心モチーフの出所 / 意図的に避けた既存曲 / 不安のある行)

**韻プロファイル heavy のとき** (song スキルと同型の分岐): 起動順を **composer ∥ rapper (subagent_type: `suno-artist-production:rapper`、ネタ帳モード) → lyricist** に変える。rapper のプロンプトには簡易ブリーフ全文と、絶対パスで `<プラグインルート>/skills/songwriting/references/03_rhyme-and-rhythm.md`・`09_rhyme-advanced-ja.md` (主資料。「ファイルが無ければ 03 のみで進めてよい」と添える) を渡し、返ってきた出力全文を lyricist のプロンプトに添付する (「候補の採否は自由、最終著者はあなた」と添える)。ラップ/ヒップホップ系ジャンルなら `10_hiphop-genre-ja.md` の絶対パスも composer・lyricist・rapper に渡す (「渡されたら読む。無ければ従来資料のみで可」と添える)
